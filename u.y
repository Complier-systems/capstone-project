...
assignment: 
	...
	assign_var T_ASSIGN expression								{ setVar($3, $1);
												...
												showCmd = 0;
												int_counter = 0; }
	| assign_var T_ASSIGN T_SEMICOLON						{ char* str = malloc(50);
												sprintf(str, "Missing operand (line no. %d)\n",yylineno);
												yyerror(str); 
												exit(0); }
	| assign_var T_ASSIGN operator						{ char* str = malloc(50);
												sprintf(str, "Invalid expression (line no. %d)\n",yylineno);
												yyerror(str); 
												exit(0); }
...
expression: 
	...
	| expression T_PLUS T_SEMICOLON						{ char* str = malloc(50);
												sprintf(str, "Missing operand (line no. %d)\n",yylineno);
												yyerror(str); 
												exit(0); }

	| expression T_MINUS T_SEMICOLON						{ char* str = malloc(50);
												sprintf(str, "Missing operand (line no. %d)\n",yylineno);
												yyerror(str); 
												exit(0); }

	| expression T_MULTIPLY T_SEMICOLON						{ char* str = malloc(50);
												sprintf(str, "Missing operand (line no. %d)\n",yylineno);
												yyerror(str); 
												exit(0); }

	| expression T_DIVIDE T_SEMICOLON						{ char* str = malloc(50);
												sprintf(str, "Missing operand (line no. %d)\n",yylineno);
												yyerror(str); 
												exit(0); }

	| expression T_MOD T_SEMICOLON	        				{ char* str = malloc(50);
												sprintf(str, "Missing operand (line no. %d)\n",yylineno);
												yyerror(str); 
												exit(0); }
